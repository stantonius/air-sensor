...
class Pressure(LPS22HB):
    """
    Modified code from examples found at https://www.waveshare.com/wiki/Sense_HAT_(B)
    I don't pretend to know what any of the C (?) code does above
    """
    ...
    def _get_reading(self):
        ...
        if (self._read_byte(LPS_STATUS)&0x01)==0x01:  # a new pressure data is generated
            self.u8Buf[0]=self._read_byte(LPS_PRESS_OUT_XL)
            self.u8Buf[1]=self._read_byte(LPS_PRESS_OUT_L)
            self.u8Buf[2]=self._read_byte(LPS_PRESS_OUT_H)
            self.PRESS_DATA=((self.u8Buf[2]<<16)+(self.u8Buf[1]<<8)+self.u8Buf[0])/4096.0
